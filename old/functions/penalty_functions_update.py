...
def add_edge_traffic_times_non_simplified(
    G,
    traffic_signals_time=2,
    stop_time=2,
    turning_circle_time=0,
        ...
        if v in G.nodes and "highway" in G.nodes[v]:
            ...
            if highway_value == "traffic_signals":
                if "traffic_signals:direction" in G.nodes[v]:
                    if (
                        G.nodes[v]["traffic_signals:direction"] == "forward"
                        and G[u][v][key]["reversed"]
                    ) or (
                        G.nodes[v]["traffic_signals:direction"] == "backward"
                        and not G[u][v][key]["reversed"]
                    ):
                        traffic_time = 0
                        traffic_control = ""
                    else:
                    ...
                    if (G.nodes[v]["direction"] == "forward" and G[u][v][key]["reversed"]) or (
                        G.nodes[v]["direction"] == "backward" and not G[u][v][key]["reversed"]
                    ):
                        traffic_time = 0
                        traffic_control = ""
                    else:
                ...
                if "stop:direction" in G.nodes[v]:
                    if (G.nodes[v]["stop:direction"] == "forward" and G[u][v][key]["reversed"]) or (
                        G.nodes[v]["stop:direction"] == "backward" and not G[u][v][key]["reversed"]
                    ):
                        traffic_time = 0
                        traffic_control = ""
                    else:
                ...
                for m, next_edge_keys in G[v].items():
                    ...
                    for next_key in next_edge_keys:
                        ...
                        if "bearing" in G[u][v][key] and "bearing" in G[v][m][next_key]:
                            ...
                            bearing_diff = (
                                G[v][m][next_key]["bearing"] - G[u][v][key]["bearing"]
                            ) % 360
...
def get_slight_turn_penalty_dict(
    G,
    left_turn_penalty=30,
    slight_left_turn_penalty=20,
    right_turn_penalty=10,
    slight_right_turn_penalty=20,
    u_turn_penalty=90,
):
    """Calculate turn penalties for different types of turns at intersections in a graph.
    ...
    **kwargs : keyword arguments
    ...
    for i in range(len(origins_gdf)):
        ...
        orig_node = ox.distance.nearest_nodes(
            G,
            origins_gdf.iloc[i]["geometry"].x,
            origins_gdf.iloc[i]["geometry"].y,
        )
        dest_node = ox.distance.nearest_nodes(
            G,
            destinations_gdf.iloc[i]["geometry"].x,
            destinations_gdf.iloc[i]["geometry"].y,
        )
