...
def calculate(G, source, target, turn_penalty):
    ...
    """
    try:
        penalty_route = shortest_path_turn_penalty(
            G,
            source,
            target,
            weight="total_time",
        ...
        penalized_total_time = sum(
            ox.utils_graph.get_route_edge_attributes(G, penalty_route, "total_time"),
        )
        penalized_distance = sum(
            ox.utils_graph.get_route_edge_attributes(G, penalty_route, "length"),
        )
    except:
    ...
    try:
        freeflow_route = shortest_path_turn_penalty(
            G,
            source,
            target,
            weight="travel_time",
        ...
        freeflow_travel_time = sum(
            ox.utils_graph.get_route_edge_attributes(G, freeflow_route, "travel_time"),
        )
        freeflow_distance = sum(
            ox.utils_graph.get_route_edge_attributes(G, freeflow_route, "length"),
        )
    except:
        # If the path is unsolvable, return -1, -1
    ...
    try:
        ...
        shortest_path_travel_time = sum(
            ox.utils_graph.get_route_edge_attributes(G, distance_route, "travel_time"),
        )
        shortest_path_distance = sum(
            ox.utils_graph.get_route_edge_attributes(G, distance_route, "length"),
        )
    except:
        # If the path is unsolvable, return -1, -1
