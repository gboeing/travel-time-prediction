...
def calculate(G, gdf_nodes, source, target, turn_penalty, sy, sx, ty, tx, hod, uber_time, algorithm='penalized',
              left_turn_penalty=5.7, right_turn_penalty=7.7, u_turn_penalty=11.1):
    """
    ...
    try:
        ...
        travel_time = sum(ox.utils_graph.get_route_edge_attributes(G, route, 'total_time'))
        total_time_with_turn_penalty = travel_time + left_turn_penalty * left_count + right_turn_penalty * right_count + u_turn_penalty * u_count
    ...
    return (source, target, sy, sx, ty, tx, hod, uber_time, travel_time, total_time_with_turn_penalty,
...
def run():
    ...
    all_OD_pairs = pd.read_csv(output_file_path + 'OD3am_10000.csv')
    # a subset of the sampled OD pairs, as we may spread ran it across machines
    OD_pair_sample = all_OD_pairs[:100]

    G = ox.io.load_graphml(output_file_path + 'LA_clip_convex_strong_network.graphml')

    # set up turn penalty dictionary
    turn_penalty = get_turn_penalty_dict(G, left_turn_penalty=5.7, right_turn_penalty=7.7, u_turn_penalty=11.1)
    ...
    args = (
        (G, gdf_nodes, OD_pair_sample.iloc[i]['oid'], OD_pair_sample.iloc[i]['did'], turn_penalty,
         ...
         OD_pair_sample.iloc[i]['hod'], OD_pair_sample.iloc[i]['mean_travel_time'], routing_algorithm
         )
    ...
    df = pd.DataFrame(all_results, columns=['oid', 'did', 'oy', 'ox', 'dy', 'dx', 'hour_of_day', 'uber_time',
                                            'travel_time', 'total_time_with_turn_penalty', 'distance',
                                            'route',
                                            'signal_count',
    ...
    df.to_csv(output_file_path + f"{routing_algorithm}_try_1217.csv")
