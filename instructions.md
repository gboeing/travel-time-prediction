...
**(New) Fast Test Mode:**
To verify the code runs without errors using a small subset of data (runs in ~1 minute):
`source one_click_run.sh --test`

**(New) Model Only Mode:**
If you have already run the routing step and only want to retrain models:
...
### Step 1 & 2 (Automated)
  - The script will preprocess input data and re-query Google Routes API.
