...
def get_routes_from_gdfs(G, origins_gdf, destinations_gdf, weight='weight', penalty={}):
  ...
  This function finds the nearest network nodes for each origin and destination point.
  Then calculates the routes using the specified graph, edge weight, and optional turn penalties using shortest_path_turn_penalty function.
  ...
  weight : str, optional
      The edge weight attribute to consider when finding routes (default: 'weight').
  penalty : dict, optional
      A dictionary containing turn penalties for specific intersections (default: empty dictionary).
      The turn penalties can be calculated using the 'get_turn_penalty_dict' function.
